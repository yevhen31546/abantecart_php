...
//
$_CONFIG['main_title'] = "Images Manager";
...
//
$_CONFIG['users'] = array(array("jchen1", "u", "admin"),array("M0", "u", "admin"),array("Jiang02", "u", "admin"),array("Sonny1", "u", "admin"),array("Carole25", "u", "admin"),array("claudi8", "u", "admin"),array("yunzhu7", "u", "admin"),);
...
$_TRANSLATIONS["pl"] = array(
	...
	"total_used_space" => "Zajęta przestrzeń",
	...
	"location" => "Miejsce",
	"root" => "Root",
	"log_file_permission_error" => "Brak uprawnień aby utowrzyć dziennik działań.",
	"upload_not_allowed" => "Konfiguracja zabrania przesłania pliku do tego folderu.",
	"upload_dir_not_writable" => "Nie można zapisac pliku do tego folderu.",
	"mobile_version" => "Wersja Mobilna",
	"standard_version" => "Widok Standardowy",
	"page_load_time" => "Załadowano w %.2f ms",
	"wrong_pass" => "Nie poprawna nazwa użytkownika lub hasło",
	"username" => "Użytkownik",
	"log_in" => "Zaloguj się",
	"upload_type_not_allowed" => "Ten rodzaj pliku jest zabrioniony.",
	"del" => "Kasuj",
	"log_out" => "Wyloguj"
);
...
class eeLogger
{
	...
	public static function logQuery()
	{
		...
		{
			eeLogger::logAccess($_POST['log'], false);
			return true;
		}
		else
			return false;
...
class GateKeeper
{
	...
	public static function isUploadAllowed(){
		...
		//* custom code start: allow uploading without login.
		//* changed below line.
	...
	public static function isNewdirAllowed(){
		...
		// if(EncodeExplorer::getConfig("newdir_enable") == true)
			return true;
		return false;
	}
	...
	public static function isDeleteAllowed(){
		...
		if(EncodeExplorer::getConfig("delete_enable") == true)
			return true;
		return false;
	}
...
class FileManager
{
	...
	*/
	function newFolder($location, $dirname)
	{
		...
		if(strlen($dirname) > 0)
		{
			...
			else
			{
				...
				eeLogger::logCreation($location->getDir(true, false, false, 0).$dirname, true);
				eeLogger::emailNotification($location->getDir(true, false, false, 0).$dirname, false);
			}
		}
	}

	public static function IsExists($location, $name)
	{
	...
	function uploadFile($location, $userfile)
	{
		...
		else
		{
			...
			eeLogger::logCreation($location->getDir(true, false, false, 0).$name, false);
			eeLogger::emailNotification($location->getDir(true, false, false, 0).$name, true);
		}
	}
	...
	public static function rename_file($file,$newname){
		if(is_file($file)){
			rename($file,$newname);
		}
	}
	...
	*/
	public static function checkFile() {
		...
		if (isset($_POST['userfile']) && count($_POST['userfile']) > 0)
		{
			$location = new Location();
			$location->init();

			$dup = 0;
			foreach ($_POST['userfile'] as $file) {
				if (FileManager::IsExists($location, $file)) {
					$dup = 1;
				}
			}
			echo $dup;
	...
	function run($location)
	{
		...
		if(isset($_FILES['userfile']['name']) && count($_FILES['userfile']['name']) > 0){
			if($location->uploadAllowed() && GateKeeper::isUserLoggedIn() && GateKeeper::isAccessAllowed() && GateKeeper::isUploadAllowed()){
				...
				{
					$eachFile = array();
					$eachFile['name'] = $_FILES['userfile']['name'][$i];
					$eachFile['type'] = $_FILES['userfile']['type'][$i];
					$eachFile['tmp_name'] = $_FILES['userfile']['tmp_name'][$i];
					$eachFile['error'] = $_FILES['userfile']['error'][$i];
					$eachFile['size'] = $_FILES['userfile']['size'][$i];

					// check overwrite files
					if ($_POST['overwrite'] == 0 && FileManager::IsExists($location, $eachFile['name']))
						continue;

					$this->uploadFile($location, $eachFile);
				}
			}
		}
		// custom code end
...
?>
	...
	$('td.ren a').click(function(oldname) {
			var newname = prompt("Please enter new name", oldname);
			if (newname != null && newname != oldname) {return newname};
	});
<?php
	}
...
?>
		$("#upload_dialog").click(function(e) {
			$("#file_input").click();
		});
		...
		$("#file_input").change(function(e) {
			...
			$.ajax({
					...
					success: function(res) {
						$("#overwrite").val('1');
						if (res == '1') {
							if (confirm('The file already exists. Do you want to overwrite it?') == 0)
								$("#overwrite").val('0');
						}
						$("#btn_upload").click();
					},
...
<div id="frame">
<?php
if(EncodeExplorer::getConfig('show_top') == true && GateKeeper::isAccessAllowed())
{
?>
<div id="top">
<table border=0><tr>
	<!--<a href="<?php print $this->makeLink(false, false, null, null, null, ""); ?>">-->
	<td><a href="#" id="upload_dialog">	Upload Files </a></td>

	<!-- START: Upload area -->
<td><form enctype="multipart/form-data" name="frm_upload" method="post">
	<div id="upload">
		<?php
		...
		<div id="upload_container">
			<input name="userfile[]" id="file_input" type="file" multiple="multiple" class="upload_file" />
			<input type="submit" value="<?php print $this->getString("upload"); ?>" id="btn_upload" class="upload_sumbit" />
		</div>
		<?php
		}
		?>
...
<tr class="row one header">
	...
	<?php } ?>
	<?php if($this->mobile == false && GateKeeper::isDeleteAllowed()){?>
	<!--td class="ren"><?php //print EncodeExplorer::getString("ren"); ?></td-->
	<?php } ?>
</tr>
<?php
}
...
if($this->dirs)
{
	foreach ($this->dirs as $dir)
	{
		...
		if($this->mobile == false && GateKeeper::isDeleteAllowed())
		{
			...
			//print "<td class=\"ren\"><a data-name=\"".htmlentities($dir->getName())."\" href=\"".$this->makeLink(false, false, null, null, $this->location->getDir(false, true, false, 0).$dir->getNameEncoded(), $this->location->getDir(false, true, false, 0))."\"><img src=\"?img=ren\" alt=\"Rename\" /></a></td>";
		}
		print "</tr>\n";
		$row =! $row;
	}
...
<a href="http://www.123filter.com">iSpring File Manager</a>
</div>
<!-- END: Info area -->
</body>
</html>
...
if(!ImageServer::showImage() && !eeLogger::logQuery() && !FileManager::checkFile())
{
	$location = new Location();
	$location->init();
	if(GateKeeper::isAccessAllowed())
	{
		eeLogger::logAccess($location->getDir(true, false, false, 0), true);
