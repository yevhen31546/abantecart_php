...
/*		if($this->model_catalog_product->isStockTrackable($product_id)){
			...
			if ($_SERVER['REMOTE_ADDR'] == '24.98.164.254') {echo '<H2>'. $product_info['quantity'] .'</H2>'; }
Commented out for displaying "Out of Stock" */
			if($product_info['quantity'] <= 0 ){
				$this->data['can_buy'] = false;
				$this->data['in_stock'] = false;
				//show out of stock message
				$this->data['stock'] = $this->language->get('text_out_of_stock');
				if ( strstr($product_info['date_available'],'0000-')==false ) $this->data['stock'] .= ' | ETA: '.  date('M/d/Y',strtotime($product_info['date_available']));
			} else{
				$this->data['can_buy'] = true;
