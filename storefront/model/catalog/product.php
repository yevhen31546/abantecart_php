...
class ModelCatalogProduct extends Model{
	...
	public function getProduct($product_id){
		...
		$query = $this->db->query(
				...
				WHERE p.product_id = '" . (int)$product_id . "'
						...
						AND  p.status = '1'");
						//~ AND p.date_available <= NOW() AND p.status = '1'"); Commented out for displaying "Out of Stock | ETA: date_available"
		return $query->row;
	}
	...
	public function getProductDataForCart($product_id){
		...
		$query = $this->db->query(
				...
				WHERE p.product_id = '" . (int)$product_id . "'
				-- commented out for still listing the product in the category -- AND p.date_available <= NOW()
				AND p.status = '1'
				");
	...
	public function getTotalProductsByManufacturerId($manufacturer_id = 0){
		$query = $this->db->query("SELECT COUNT(*) AS total
									...
									WHERE status = '1'
											-- commented out for still listing the product in the category -- AND date_available <= NOW()
	...
	public function getProductsByTag($tag, $category_id = 0, $sort = 'p.sort_order', $order = 'ASC', $start = 0, $limit = 20){
		if ($tag){
			...
			//~ $sql .= " AND p.status = '1' AND p.date_available <= NOW() GROUP BY p.product_id"; -- commented out for still listing the product in the category --
	...
	public function getTotalProductsByKeyword($keyword, $category_id = 0, $description = false, $model = false){
		...
		if ($keyword){
			...
			// $sql .= " AND p.status = '1' AND p.date_available <= NOW()"; -- commented out for still listing the product in the category --
			$sql .= " AND p.status = '1' ";
			$query = $this->db->query($sql);
			if ($query->num_rows){
