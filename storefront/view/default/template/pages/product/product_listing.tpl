<div class="thumbnails grid row list-inline">
	<?php
	$trusted_ips = '98.192.10.88,73.106.162.5';
	$icount = 0;
	...
	foreach ($products as $product) {
		...
		if (stristr($product['blurb'],'<li>')<>false) $product['blurb'] = '<OL>'. $product['blurb'] .'</OL>';
		...
		<div class="col-md-3 col-sm-6 col-xs-12">
			...
			<div class="thumbnail">
				...
				<div class="shortlinks" style="text-align: left;">
					...
					echo '<br>'. substr(str_ireplace('<OL>','<UL>',$product['blurb']),0,256); //John Chen 2018-09-13 Add blurb to the pop-up
					?>
				</div>
				<div class="blurb"><?php //John Chen 2018-09-13 blurb makes the height uneven. echo $product['blurb'] ?></div>
...
<div class="thumbnails list row listing-product">
	...
	foreach ($products as $product) {
		...
		if ($product['special']) continue; //John Chen 2018-09-13 messy to include the product detail page in the product list page
		?>
		<div>
			<div class="thumbnail">
				<div class="row">
					...
					<div class="col-md-8">
						...
						<!--<div class="productdiscrption">-->
							...
							<!--</div>-->
						<div class="blurb"><?php echo htmlspecialchars_decode($product['blurb']); ?></div>
...
.listing-product .thumbnail .blurb{
max-height: 200px;
text-align: left;
}
.listing-product .thumbnail .blurb ul {
padding-left: 17px;
}
.listing-product .thumbnail .shortlinks{
float: right;
}
.listing-product .thumbnail .pricetag_wide{
clear: both;
}
.listing-product .thumbnail .pricetag_wide .price{
float: right;
margin-right: 30px;
}
.listing-product .thumbnail .price .oneprice{
font-size: 16px;
}
.listing-product .thumbnail .row{
