	...
	public function getOrders(){
	    ...
	    foreach ($orders->rows as $order) {
		    ...
		    if($fraudlabs->num_rows > 0){
				...
				$data = '<a href="http://www.fraudlabspro.com/merchant/transaction-details/'.$fraudlabs->row['fraudlabspro_id'].'">'.$order['order_id'].', '.$fraudlabs->row['fraudlabspro_score'].', '.$fraudlabs->row['fraudlabspro_status'].$fraud.'</a>';
			}else{
				$data = '';
			}
