...
class ModelShipstationXmlOrders extends Model {
	...
	public function getOrders(){
	    ...
	    foreach ($orders->rows as $order) {
		    ...
		    if($fraudlabs->num_rows > 0){
		    	$fraud = '';
		    	if($fraudlabs->row['fraudlabspro_score'] > 70){
		    		$fraud = ',Fraud';
		    	}
				//~ over 100 char $data = '<a href="http://www.fraudlabspro.com/merchant/transaction-details/'.$fraudlabs->row['fraudlabspro_id'].'">'.$order['order_id'].', '.$fraudlabs->row['fraudlabspro_score'].', '.$fraudlabs->row['fraudlabspro_status'].$fraud.'</a>';
				$data = 'www.fraudlabspro.com/merchant/transaction-details/'.$fraudlabs->row['fraudlabspro_id'];
			}else{
				$data = '';
			}
